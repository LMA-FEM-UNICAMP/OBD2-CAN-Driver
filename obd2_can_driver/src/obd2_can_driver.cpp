...
Obd2CanDriver::Obd2CanDriver(std::string can_in, std::string can_out)
{
    ...
    can_type_.store(CAN_TYPE_UNKOWN);
...
bool Obd2CanDriver::obd2_request(uint8_t pid)
{
    ...
    /// Sending OBD2 request for the desired PID in the right CAN format

    if (can_type_.load() == IS_STANDARD_CAN || can_type_.load() == CAN_TYPE_UNKOWN)
    {
        request_frame.can_id = 0x7DF; // Broadcast ID for Standard CAN

        sendbytes = write(socket_in_, &request_frame, sizeof(can_frame_t));
    }

    if (can_type_.load() == IS_EXTENDED_CAN || can_type_.load() == CAN_TYPE_UNKOWN)
    {
        request_frame.can_id = 0x18DB33F1 | CAN_EFF_FLAG; // Broadcast ID for Extended CAN
...
can_frame_t Obd2CanDriver::obd2_response()
{
    ...
    if (response_frame.can_id & CAN_EFF_FLAG)
    {
        can_type_.store(IS_EXTENDED_CAN);
    }
    else
    {
        can_type_.store(IS_STANDARD_CAN);
    }

    if (can_type_.load() == IS_STANDARD_CAN)
    {
        /// Checking if the response was successful, the ID is valid
        if (nbytes > 0 && ((response_frame.can_id & CAN_SFF_MASK) & 0xFF0) == 0x7E0)
        {
            return response_frame;
        }
    }
    else if (can_type_.load() == IS_EXTENDED_CAN)
    {
        /// Checking if the response was successful, the ID is valid
        if (nbytes > 0 && ((response_frame.can_id & CAN_EFF_MASK) & 0xFFFFFF00) == 0x18DB3300)
        {
            return response_frame;
        }
    }
...
bool Obd2CanDriver::send_data_to_can_out()
{
    ...
    if (is_new_data_)
    {
        ...
        u_int16_t longitudinal_acceleration_mm_per_s2 = 161; // longitudinal_acceleration_ in m/s^2 to mm/s^2 (161 - unavailable)
